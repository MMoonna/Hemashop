...
import { changeCartAPI, deleteCartAPI, myCartAPI } from '@/services/cart'
...
const onChangeCount = (e: InputNumberBoxEvent) => {
  changeCartAPI(e.index, { count: e.value })
}
const changeChecked = (item: CartItem) => {
  item.selected = !item.selected
  changeCartAPI(item.skuId, { selected: item.selected })
}
const selectedAll = computed(() => {
  return cartList.value.length && cartList.value.every((item) => item.selected)
})
const changeAllChecked = () => {
  const reselectedAll = !selectedAll.value
  cartList.value.forEach((item) => {
    item.selected = reselectedAll
  })
}
// 计算选中单品列表
...
const selectedTotal = computed(() => {
  return selectedList.value.reduce((total, item) => (total += item.count), 0)
})
// 计算选中商品总价格
...
<template>
  <scroll-view scroll-y class="scroll-view">
    ...
    <template v-if="memberInfo.profile">
      ...
      <view class="cart-list" v-if="true">
        ...
        <uni-swipe-action>
          ...
          <uni-swipe-action-item v-for="item in cartList" :key="item.skuId" class="cart-swipe">
            <!-- 商品信息 -->
            <view class="goods" @tap="changeChecked(item)">
              <!-- 选中状态 -->
            ...
            <template #right>
              <view class="cart-swipe-right">
                <button class="button delete-button" @tap="deleteCart([item.skuId])">删除</button>
              </view>
            </template>
      ...
      <!-- 吸底工具栏 -->
      <view class="toolbar">
        <text class="all" :class="{ checked: selectedAll }" @tap="changeAllChecked">全选</text>
